...
本机 docker 起一个独立的 ccr-app, 内含 claude code CLI, 反向连接 vibe.qpqi.group
...
浏览器进 https://vibe.qpqi.group → 登录 → DevTools 控制台:
```js
...
```bash
curl -X POST https://vibe.qpqi.group/api/hub/pair/redeem \
...
CCR_HUB_URL=wss://vibe.qpqi.group
...
- 浏览器 https://vibe.qpqi.group → 应看到两个 app online: UbuntuClaw + ClawDocker
