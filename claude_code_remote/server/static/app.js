...
// 注册 — 用户已经登录, 点 Settings 内 "Add passkey".
async function passkeyRegister(nickname) {
  if (!window.PublicKeyCredential) {
    throw new Error("This browser doesn't support passkeys.");
  }
  const opts = await api("/api/hub/passkey/register/start", {
    method: "POST",
    body: JSON.stringify({ nickname: nickname || "" }),
  });
  // 把 base64url string 字段转回 ArrayBuffer (WebAuthn JS API 要 binary)
  opts.challenge = _b64uToU8(opts.challenge);
  opts.user.id = _b64uToU8(opts.user.id);
  if (opts.excludeCredentials) {
    opts.excludeCredentials = opts.excludeCredentials.map(c =>
      Object.assign({}, c, { id: _b64uToU8(c.id) }));
  }
  const cred = await navigator.credentials.create({ publicKey: opts });
  if (!cred) throw new Error("Registration cancelled");
  const payload = {
    id: cred.id,
    rawId: _u8ToB64u(cred.rawId),
    type: cred.type,
    response: {
      clientDataJSON: _u8ToB64u(cred.response.clientDataJSON),
      attestationObject: _u8ToB64u(cred.response.attestationObject),
    },
  };
  await api("/api/hub/passkey/register/finish", {
    method: "POST",
...
async function passkeyLogin() {
  ...
  const opts = await api("/api/hub/passkey/login/start", { method: "POST" });
  opts.challenge = _b64uToU8(opts.challenge);
  ...
  const payload = {
    ...
    response: {
      clientDataJSON: _u8ToB64u(cred.response.clientDataJSON),
      authenticatorData: _u8ToB64u(cred.response.authenticatorData),
      signature: _u8ToB64u(cred.response.signature),
      userHandle: cred.response.userHandle
        ? _u8ToB64u(cred.response.userHandle) : null,
    },
  };
  await api("/api/hub/passkey/login/finish", {
    method: "POST",
    body: JSON.stringify({ credential: payload }),
  });
  // 重新探一次 me 同步登录态
...
const _loginPasskeyBtn = $("login-passkey");
if (_loginPasskeyBtn) _loginPasskeyBtn.addEventListener("click", async () => {
  $("login-err").classList.remove("show");
  _loginPasskeyBtn.disabled = true;
  try {
    await passkeyLogin();
    enterHome();
  } catch (e) {
    $("login-err").textContent = "Passkey sign-in failed: " + (e.message || e);
    $("login-err").classList.add("show");
  } finally {
    _loginPasskeyBtn.disabled = false;
  }
});
...
// ---------- Passkey list / add in Settings (hub mode) ----------
(function setupPasskeyMgmt() {
  const listEl = $("settings-passkey-list");
  const addBtn = $("settings-passkey-add");
  if (!listEl || !addBtn) return;

  async function reload() {
    if (!state.hubMode || !state.userId) {
      listEl.innerHTML = '<div class="apps-empty">Sign in to manage passkeys.</div>';
      return;
    }
    try {
      const rows = await api("/api/hub/passkeys");
      if (!rows || !rows.length) {
        listEl.innerHTML = '<div class="apps-empty">No passkeys yet.</div>';
        return;
      }
      listEl.innerHTML = "";
      const now = Date.now() / 1000;
      for (const k of rows) {
        ...
        const name = k.nickname || k.id.slice(0, 10) + "…";
        const created = k.created_at
          ? Math.floor((now - k.created_at) / 86400) + "d ago"
          : "?";
        const last = k.last_used_at
          ? Math.floor((now - k.last_used_at) / 86400) + "d ago"
          : "never used";
        row.innerHTML = `
          <span class="state-dot" style="background:var(--accent,#3fb950)" aria-hidden="true"></span>
          <div class="app-rows">
            <div class="app-row-1"><span class="app-name"></span></div>
            <div class="app-row-2">
              <span class="app-stat">Added ${escHTML(created)}</span>
              <span class="sep">·</span>
              <span class="app-stat">Last used ${escHTML(last)}</span>
            </div>
          </div>
          <button class="app-revoke" type="button">Revoke</button>`;
        row.querySelector(".app-name").textContent = name;
        row.querySelector(".app-revoke").addEventListener("click", async () => {
          if (!confirm(`Revoke passkey "${name}"?`)) return;
          try {
            await api(`/api/hub/passkeys/${encodeURIComponent(k.id)}`, {
              method: "DELETE",
            });
            reload();
          } catch (e) {
      ...
      listEl.innerHTML = '<div class="apps-empty">Failed to load: '
  ...
  addBtn.addEventListener("click", async () => {
    const nickname = prompt(
      "Name this passkey (e.g. 'iPhone 15 Pro', 'Mac mini', 'YubiKey 5C'):",
      "",
    );
    if (nickname === null) return;   // user cancelled
    addBtn.disabled = true;
    addBtn.textContent = "Waiting for biometric…";
    try {
      await passkeyRegister((nickname || "").trim());
      reload();
    } catch (e) {
      alert("Add passkey failed: " + (e.message || e));
    } finally {
      addBtn.disabled = false;
      addBtn.textContent = "+ Add a passkey on this device";
    }
  });

  // settings-btn 点击时也 reload (重新打开 settings 拉一次最新)
  const settingsBtn = $("settings-btn");
  if (settingsBtn) settingsBtn.addEventListener("click", () => {
    if (state.hubMode && state.userId) reload();
  });
})();
